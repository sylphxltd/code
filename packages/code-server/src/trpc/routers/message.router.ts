...
const StreamEventSchema = z.discriminatedUnion("type", [
	...
	z.object({
		type: z.literal("session-title-updated-start"),
		sessionId: z.string(),
	}),
	z.object({
		type: z.literal("session-title-updated-delta"),
		sessionId: z.string(),
		text: z.string(),
	}),
	z.object({
		type: z.literal("session-title-updated-end"),
		sessionId: z.string(),
		title: z.string(),
	}),
	z.object({
		type: z.literal("session-model-updated"),
		sessionId: z.string(),
		model: z.string(),
	}),
	z.object({
		type: z.literal("session-provider-updated"),
		sessionId: z.string(),
		provider: z.string(),
		model: z.string(),
	}),
	...
	add: moderateProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			try {
				...
				console.log("[message.router] ✅ Event published successfully to channel:", sessionId);
			} catch (error) {

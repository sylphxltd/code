...
export const globTool = tool({
	...
	inputSchema: z.object({
		pattern: z
			.string()
			.describe(
				'Glob pattern: * (any except /), ** (any), ? (single). Examples: "*.ts", "src/**/*.tsx"',
			),
...
export const grepTool = tool({
	...
	inputSchema: z.object({
		...
		type: z
			.string()
			.optional()
			.describe("File type: js, ts, py, rust, go, java, c, cpp, html, css, json, yaml, md"),
						...
						if (regex.test(lines[i])) {
							...
							if (output_mode === "content") {
								...
								for (let j = startLine; j <= endLine; j++) {
									const lineContent = showLineNumbers ? `${j + 1}: ${lines[j]}` : lines[j];

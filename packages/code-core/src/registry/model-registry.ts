 ...
 */
export function getModelWithProvider(modelId: string): (Model & { provider: Provider }) | undefined {
 ...
 */
export function modelSupportsInput(modelId: string, capability: keyof Model['inputCapabilities']): boolean {
  const model = MODELS[modelId];
 ...
 */
export function modelSupportsOutput(modelId: string, capability: keyof Model['outputCapabilities']): boolean {
  const model = MODELS[modelId];

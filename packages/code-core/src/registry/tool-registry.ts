...
export const TOOLS = {
	...
	read: {
		...
		category: "filesystem",
		description:
			"Read contents of a file from the filesystem. Supports pagination with offset and limit parameters for large files.",
		capabilities: {
			isAsync: true,
	...
	write: {
		...
		source: "builtin",
		relatedTools: ["read"],
		examples: ['write({ file_path: "config.json", content: "{\\"key\\": \\"value\\"}" })'],
	},
	...
	bash: {
		...
		category: "shell",
		description:
			"Execute a bash command and return output. Supports background execution and timeouts.",
		capabilities: {
	...
	grep: {
		...
		category: "search",
		description:
			"Search for patterns in files using ripgrep. Supports regex, glob filters, and context lines.",
		capabilities: {
			isAsync: true,
			supportsParallel: true,
		},
	...
	glob: {
		...
		source: "builtin",
		relatedTools: ["grep"],
		examples: ['glob({ pattern: "**/*.ts" })', 'glob({ pattern: "src/**/*.test.ts" })'],
	},
	...
	ask: {
		...
		description:
			"Ask the user a question and get their response. Supports multiple choice and free text.",
		capabilities: {
			isAsync: true,
			isStateful: true,
			supportsParallel: false,
		...
		examples: ['ask({ questions: [{ question: "Which approach?", options: [...] }] })'],
	},
...
export function getToolsSupportedByModel(modelId: string): Tool[] {
	return Object.values(TOOLS).filter((tool) => isToolSupportedByModel(tool.id, modelId));
}

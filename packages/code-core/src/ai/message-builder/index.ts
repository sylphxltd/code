			...
			for (const part of step.parts) {
				if (part.type === "text") {
					...
					if (!fileContent) {
						...
						contentParts.push({
							type: "text",
							text: `[Error: File not found: ${part.relativePath}]`,
						});
						continue;
					}
...
async function buildAssistantMessage(
	...
	if (steps.length > 0) {
		...
		if (steps.length > 1) {
			console.warn(
				`[buildAssistantMessage] WARNING: Received ${steps.length} steps, but should only process one step at a time. Processing first step only.`,
			);
		}
			...
			switch (part.type) {
				...
				case "tool":
					...
					assistantContent.push({
						...
						toolName: part.name,
						input: part.input || {}, // ← AI SDK uses 'input'
					} as ToolCallPart);
